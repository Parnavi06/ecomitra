...
import { LayoutDashboard, Map as MapIcon, List, User as UserIcon, LogOut, Trash2 } from 'lucide-react';
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ user, logout }) => {
    ...
    return (
        <aside className="flex w-72 bg-emerald-950 border-r border-emerald-900/50 text-emerald-100 flex-col h-full shrink-0 shadow-2xl relative z-50">
            <div className="p-8">
            ...
            <nav className="flex-1 px-6 space-y-1.5 mt-4">
                <AdminSidebarLink to="/admin" active={location.pathname === '/admin'} icon={<LayoutDashboard size={18} />} label="Dashboard" />
                <AdminSidebarLink to="/admin/locations" active={location.pathname === '/admin/locations'} icon={<MapIcon size={18} />} label="Locations" />
                <AdminSidebarLink to="/admin/bins" active={location.pathname === '/admin/bins'} icon={<List size={18} />} label="Bins" />
                <AdminSidebarLink to="/admin/profile" active={location.pathname === '/admin/profile'} icon={<UserIcon size={18} />} label="Profile" />
            </nav>

            <div className="p-6 m-4 bg-emerald-900/30 rounded-3xl border border-emerald-800/50 backdrop-blur-sm">
                <div className="flex items-center space-x-3 mb-6">
                    <img src={user?.avatar} className="w-12 h-12 rounded-2xl bg-emerald-800 border-2 border-emerald-700 shadow-lg object-cover" alt="Admin" />
                    <div className="overflow-hidden">
                        <p className="text-sm font-bold text-white truncate">{user?.name}</p>
                        <p className="text-[10px] text-emerald-400 font-black uppercase tracking-widest mt-0.5">Administrator</p>
                    </div>
                </div>
                <button
                    onClick={logout}
...
const AdminSidebarLink = ({ to, active, icon, label }: { to: string, active: boolean, icon: React.ReactNode, label: string }) => (
    <Link
        to={to}
